...
namespace xviz
{
    ...
    void Sence::DrawGrid(const Settings &settings)
    {
        ...
        {
            g_debugDraw.DrawSegment(b2Vec2(x, 0), b2Vec2(x, settings.m_gridHeight), B2WHITE, 0.05);
        }

        for (float y = 0; y <= settings.m_gridHeight; y += settings.m_gridInterval)
        {
            g_debugDraw.DrawSegment(b2Vec2(0, y), b2Vec2(settings.m_gridWidth, y), B2WHITE, 0.05);
        }
    }
    void Sence::DrawOrigin()
    {
        g_debugDraw.DrawSegment(b2Vec2(0, 0), b2Vec2(5.0f, 0), B2GREEN, 0.05);
        g_debugDraw.DrawSegment(b2Vec2(0, 0), b2Vec2(0, 5.0), B2RED, 0.05);
    }
